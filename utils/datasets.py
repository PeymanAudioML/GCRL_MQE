...
class Dataset:
    ...
    def __init__(self, data: dict):
        ...
        if 'valids' in data:
            (self.valid_idxs,) = np.nonzero(data['valids'] > 0)
    ...
    def get_subset(self, idxs):
        ...
        result = jax.tree_util.tree_map(lambda arr: arr[idxs], self._data)
        if 'next_observations' not in result:
            result['next_observations'] = self._data['observations'][np.minimum(idxs + 1, self.size - 1)]
        return result
...
class ReplayBuffer(Dataset):
    ...
    def add_transition(self, transition):
        ...
        jax.tree_util.tree_map(set_idx, self._data, transition)
    ...
    def __post_init__(self):
        ...
        if self.config['frame_stack'] is not None:
            ...
            if self.preprocess_frame_stack:
                ...
                self.dataset = Dataset({**self.dataset._data, 'observations': stacked_observations})
